...
if ($showSimilar) {
    ...
    $simStmt  = db()->prepare("
        SELECT p.*,
               ...
               (p.city = ?) * 2 + (p.property_type = ?) * 1 AS score
          FROM properties p
          JOIN landlords l ON l.user_id = p.landlord_id
         WHERE p.id != ?
           AND p.status = 'available'
    ...
    ");
    $simStmt->execute([
        $prop['city'], $prop['property_type'], $prop['id'],
        $rentLow, $rentHigh, $prop['monthly_rent'],
    ]);
    ...
    <aside class="property-rail d-none d-lg-flex flex-column">
        ...
        <div class="rail-actions">
            <?php if (is_logged_in()): ?>
                ...
                <!-- Report listing (not available to the landlord who owns it) -->
                <?php if (current_user_id() !== (int)$prop['landlord_user_id']): ?>
                    <button type="button"
                            class="btn btn-outline-danger rail-btn-secondary"
                            onclick="bootstrap.Modal.getOrCreateInstance(document.getElementById('reportModal')).show()">
                        <i class="bi bi-flag me-1"></i> Report listing
                    </button>
                <?php endif; ?>
...
.sim-heading { font-family:'Fraunces',serif; font-size:1.1rem; color:#0F2C52; margin-bottom:16px; }
.sim-strip { display:flex; gap:16px; overflow-x:auto; padding-bottom:8px; scrollbar-width:thin; }
.sim-card {
    flex: 0 0 210px;
    background: white;
...
.sim-card:hover { transform:translateY(-3px); box-shadow:0 6px 16px rgba(15,44,82,0.10); }
.sim-img { aspect-ratio:4/3; background:linear-gradient(135deg,#E6ECF4,#E4F2EA); position:relative; overflow:hidden; }
.sim-img img { width:100%; height:100%; object-fit:cover; display:block; }
.sim-verified { position:absolute; top:8px; left:8px; background:#2E8B57; color:white; font-size:0.7rem; padding:2px 6px; border-radius:20px; }
.sim-body { padding:10px 12px 12px; }
.sim-type { font-size:0.72rem; text-transform:uppercase; letter-spacing:0.4px; color:#6c757d; margin-bottom:4px; display:flex; align-items:center; gap:6px; }
.sim-match-badge { background:#E4F2EA; color:#2E8B57; font-size:0.65rem; padding:1px 6px; border-radius:20px; text-transform:none; letter-spacing:0; font-weight:600; }
.sim-title { font-size:0.88rem; font-weight:600; color:#0F2C52; line-height:1.3; margin-bottom:4px; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
.sim-location { font-size:0.75rem; color:#6c757d; margin-bottom:6px; }
.sim-rent { font-family:'Fraunces',serif; font-size:1rem; font-weight:700; color:#2E8B57; }
...
<?php if (is_logged_in() && current_user_id() !== (int)$prop['landlord_user_id']): ?>
<?php render_report_modal(
    [['id' => (int)$prop['landlord_user_id'], 'name' => $prop['landlord_preferred_name'] ?: $prop['landlord_name'], 'role' => 'landlord']],
    'general',
    (int)$prop['id']
); ?>
<?php endif; ?>
